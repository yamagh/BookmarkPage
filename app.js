 ...
 * Relies on window.BookmarkUtils and window.SideEffects —
...
window.AppComponent = {
  ...
  computed: {
    ...
    tagIndex() {
      const groups = this.sortedGroupedBookmarks;
      return Object.keys(groups).map(tag => ({ tag, count: groups[tag].length }));
     }
   },
  ...
  methods: {
    ...
    scrollToTag(tag) {
      const entry = this.tagIndex.find(e => e.tag === tag);
      if (!entry) return;
      const i = this.tagIndex.indexOf(entry);
       ...
       <div class="app-root">
        ...
        <aside class="index-rail">
          ...
          <div class="rail-count">
            <span class="rail-count-num">{{ totalCount }}</span>
          ...
          <nav class="tag-index">
            ...
            <ul class="tag-index-list">
              <li v-for="(entry, i) in tagIndex" :key="entry.tag"
          ...
          <div class="rail-meta">
            <span>{{ totalCount }} bookmark{{ totalCount !== 1 ? 's' : '' }}</span>
        ...
        <main class="main">
          ...
          <div class="tag-groups" v-else>
            <ul>
              <li v-for="(group, tag, i) in sortedGroupedBookmarks" :key="tag"
                class="tag" :id="'group-' + i">
                <div class="tag-content">
                  <div class="tag-head">
                    <span class="tag-number">{{ String(i + 1).padStart(2, '0') }}</span>
                    <span class="tag-name">{{ tag }}</span>
                    <span class="tag-count">{{ group.length }}</span>
                  </div>
                  <ul>
